...
6. **Tests de integración**: `pytest -m integration` (con Postgres y Redis levantados como servicios de GitHub Actions).
7. **Escaneo de seguridad**: `pip-audit` para detectar dependencias vulnerables.
8. **Escaneo de secretos**: `gitleaks` para detectar credenciales filtradas en el diff.
